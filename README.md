...
`find()`: Возвращает первый объект, соответствующий условию.

`some() / every`(): Проверяют, подходит ли под условие хотя бы один объект или вообще все (возвращают true/false).

`sort`(): Мутирует исходный массив. Для объектов нужно передавать функцию сравнения, обращающуюся к ключам.

`forEach`(): Просто перебор. Полезен, если нужно вызвать какое-то внешнее действие для каждого объекта (например, логирование).
